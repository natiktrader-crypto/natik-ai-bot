...
# ─── WEB SEARCH (DuckDuckGo, без ключа) ──────────────────────────────────────

def _web_search(query: str, max_results: int = 4) -> str:
    """Search DuckDuckGo Instant Answer API. Returns formatted results string."""
    try:
...
def _needs_search(question: str) -> bool:
    """Heuristic: does question need live web data?"""
    keywords = [
...
def _call_claude(messages: list, timeout: int = 40) -> tuple[str, str | None]:
...
def _call_provider(key: str, messages: list, image_url: str | None = None, timeout: int = 40) -> tuple[str, str | None]:
    if key == "claude":
    ...
    # Build messages — inject system prompt at start
    ...
    # Vision: if last user message and image_url provided, add image to content
    ...
    try:
        ...
        if r.status_code not in (200, 201):
            log.warning(f"[{key}] HTTP {r.status_code}: {r.text[:150]}")
            return key, None
        return key, r.json()["choices"][0]["message"]["content"].strip()
    except Exception as e:
        log.warning(f"[{key}] {e}")
        return key, None


def _run_parallel(active_keys: list, messages: list, image_url: str | None = None) -> dict[str, str | None]:
    results = {}
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(active_keys), 12)) as ex:
        futures = {ex.submit(_call_provider, k, messages, image_url): k for k in active_keys}
...
def cmd_help(message):
    ...
    text = (
        ...
        "Отправь *текст*, *фото*, *документ* или *голосовое* — отвечают несколько AI параллельно.\n\n"
...
def cmd_keys(message):
    ...
    lines.append("_Ключи добавляй в Replit → Secrets (не пиши в чат!)_")
...
def _ask_all(message, question: str, image_url: str | None = None, extra_context: str = ""):
    uid = message.from_user.id
    active = get_active(uid)
    ...
    # Auto web search for relevant questions
    search_results = ""
    ...
    waiting_msg = bot.reply_to(
        message,
        f"⏳ Спрашиваю *{count} AI* параллельно{search_note}...\n{names}",
        parse_mode="Markdown",
    )
    ...
    answered = sum(1 for k in available if results.get(k))
    try:
        bot.send_message(
            message.chat.id,
...
# ─── DOCUMENT (PDF, images as files, etc.) ───────────────────────────────────
...
def handle_voice(message):
    bot.reply_to(
        message,
        "🎤 Голосовые сообщения пока не поддерживаются — напиши вопрос текстом.\n"
        "Или перешли аудио через @voicybot для транскрипции.",
        parse_mode="Markdown",
    )
