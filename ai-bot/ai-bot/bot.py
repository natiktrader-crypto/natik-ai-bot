...
def _call_provider(key: str, messages: list, image_url: str | None = None, timeout: int = 40) -> tuple[str, str | None, str | None]:
    if key == "claude":
...
def _run_parallel(active_keys: list, messages: list, image_url: str | None = None) -> dict[str, tuple[str | None, str | None]]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(active_keys), 12)) as ex:
        ...
        for fut in done:
            k, text, err = fut.result()
            results[k] = (text, err)
        for fut in not_done:
            k = futures[fut]
            results[k] = (None, "таймаут (>45 сек)")
    return results
